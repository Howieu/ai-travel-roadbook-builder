...
把零散旅行攻略、公开网页链接、酒店/交通截图整理成产品向网页路书的 Codex Skill + 静态站点 demo。

## 这个 repo 包含什么

- `skills/travel-roadbook-builder/`：Codex skill。它指导 Agent 从粘贴资料、Agent Reach 可读链接和用户预订截图生成 `roadbook.json`，再渲染成 HTML 路书。
- `skills/travel-roadbook-builder/scripts/render_roadbook.py`：纯 Python 标准库渲染器，输出 traveler-facing product site。
...
- `docs/`：GitHub Pages / Vercel 可部署的静态站点和生成后的 demo 页面。
- `vercel.json`：Vercel 静态部署配置，输出目录为 `docs`。
...
5. 渲染器把 JSON 转成产品向静态 HTML：首屏 CTA、执行面板、住宿/交通、来源证据、每日路线。
...
- 不做实时公共交通规划。
- 不做用户账号、社区分享或商业化。
- 不接 Supabase，除非需要保存用户行程、登录、协作、历史记录或埋点。
